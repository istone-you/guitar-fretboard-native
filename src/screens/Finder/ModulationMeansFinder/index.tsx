...
interface ModulationMeansFinderProps {
  theme: Theme;
...
export default function ModulationMeansFinder({
  theme,
  accidental,
  layers,
  globalRootNote,
  ...
  const pendingTmpLayer = useMemo(() => {
    ...
    const layer = createDefaultLayer("chord", "modulation-means-tmp", BLACK);
        ...
        <View style={styles.sectionGroup}>
          ...
          {modulationMeans.modal === null ? (
            ...
            <View style={[styles.chordRow, { borderColor, backgroundColor: colors.surface }]}>
              <View style={styles.chordLeft}>
                <Text style={[styles.chordName, { color: colors.textStrong }]}>
                  {t(MODE_I18N[modulationMeans.modal.modeName] ?? modulationMeans.modal.modeName)}
                </Text>
              </View>
            </View>
          )}
