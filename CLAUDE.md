...
- **Logic**:
  ...
  - **コード進行の度数表記**: アプリ内部では常に**大文字クロマティック度数**（`I`, `II`, `bIII`, `III`, `IV`, `bV`, `V`, `bVI`, `VI`, `bVII`, `VII`）を使用する。`CHROMATIC_DEGREES`・`DEGREE_TO_OFFSET`・`ProgressionChord.degree` はすべてこの形式。`PROGRESSION_TEMPLATES.degrees[]` のみ伝統的ダイアトニック表記（`ii`, `vi` 等の小文字）を使うが、UI や `ProgressionChord` に変換する際は必ず `resolveProgressionDegree()` を通してクロマティック度数に正規化すること。
...
│   │   ├── Templates/          # タブ3: コード進行テンプレート管理（プリセット管理はLayerタブ内）
